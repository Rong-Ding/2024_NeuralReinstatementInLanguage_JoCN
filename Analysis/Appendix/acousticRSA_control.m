function [flag] = acousticRSA_control(num)
...
trlinx_diff = trl_diff(inx_ref,inx_ref);
trlinx_same = trl_same(inx_ref,inx_ref);
...
t = -0.5:0.05:1.45;
t_pro = [-0.5:0.05:-0.05;0:0.05:0.45;0.5:0.05:0.95;1:0.05:1.45];% time axis
t_pro = t_pro(num,:);
Fs = fs;
Nspec = 0.05*Fs;
wspec = hamming(Nspec);
...
for wincnt_pro=1:length(t_pro)
    ...
    for trlcont=1:height(inx_ref)
        [S,f,T,ps] = spectrogram(wav_data.trial{1,inx_ref(trlcont)}(inx_tp_pro),wspec,Noverlap,freqRange,Fs,'power');
        %disp(T);
        mat_pro = [mat_pro ps(:)];
    end
    
    for wincnt_ref=1:length(t)
...
%% save file
datafile = ['/project/3027010.01/middata/Corr/acoustic_12122023/RSA_word_acoustic_',num2str(num),'.mat'];
