function [cell_RSA_ref,cell_RSA_pro] = data_org_words_phase_TFR(TFR,freqwin,channels,time1,time2,bas,split_band,subj)
...
samepart_ref = (sameitMat_refstory == 1) & (part_ref == part_ref');
% timing
...
% trial selection
trlinx_diff_ref = (sameitMat_refstory==1) & (sameitMat_ref==0) & (sameitMat_following~=1) & (dist_ref~=-1);
trlinx_same_ref = (sameitMat_refstory==1) & (sameitMat_ref==1) & (sameitMat_following~=1);
%% for pronouns
...
sameitMat_following = sameitMat_following | sameitMat_following';
% distance
reforder_pro = repmat(trialinfo(inx_pro,7),[1 length(inx_pro)]);
...
for j=1:(1.8/0.05) 
    ...
    for k=j:(1.8/0.05)
        ...
        set_data = TFresult_chunking_words_phase(TFR150,win1,win2,freqwin,channels,split_band);
        ...
        for l=1:size(set_data.results,2) % length = num of freq
            ...
            % RSA: referent
            CorMat_ref = circ_corrcc_mat(data_ref1, data_ref2);
            % RSA: pronoun
            CorMat_pro = circ_corrcc_mat(data_pro1, data_pro2);
