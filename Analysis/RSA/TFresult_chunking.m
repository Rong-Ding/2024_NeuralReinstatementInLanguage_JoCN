function [set_data] = TFresult_chunking(TFresult,timwin_pro,timwin_ref,freqwin,split,channels)
...
for i=1:height(freq_inx)
    ...
    cell{1,i} = log(trl); % first row: pronouns (also log-transformed)
    ...
    cell{2,i} = log(trl); % second row: referents
